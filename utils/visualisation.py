...
class Visualisation():
    ...
    def compare_model_predictions(self, x_values, y_values_list, y_actual, title):
        big_fig = go.Figure()
        big_fig.add_trace(go.Scatter(x=x_values, y=y_actual, mode='markers', name=title))
        big_fig.add_trace(go.Scatter(x=x_values, y=y_values_list, mode='markers', name=title))
        #fig.update_layout(title=title)
        #fig.show()
        #fig1.update_layout(title=title+"1")
        big_fig.show()
