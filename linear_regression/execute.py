...
logging.basicConfig(level=logging.INFO, filename="files/py_log.log", filemode="w")
MIN_MSE = 9999999999


def train_model(experiment, number_of_polynom, learning_rate, reg_coefficient, number_of_function):
    base_functions = dict([("polynom", [lambda x, i=i: x ** i for i in range(1, number_of_polynom + 1)]),
                           ("sin", [lambda x: sin(x)]),
                           ("cos", [lambda x: cos(x)]),
                           ("exp", [lambda x: exp(x)]),
                           ("log", [lambda x: log(x+0.000000000001)])
                           ])
    ...
    mse = metric.mse(prediction, data.target_valid)
    if mse < MIN_MSE:
        model_1.save('files/model.pkl')
    logging.info(f"MSE of {experiment} experiment:{metric.mse(prediction, data.target_valid)}")
    return prediction
...
for iter in range(0, 30):
    ...
    number_of_function = random.choice(["polynom", "sin", "cos", "exp", "log"])
    predictions.append(
        [
            train_model(experiment=iter,
                        number_of_polynom=number_of_polynoms,
